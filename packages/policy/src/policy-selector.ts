...
export type PolicyVersionId =
  typeof POLICY_VERSION | typeof NPR_2026_27_POLICY_VERSION;

export function policyLedgerForVersion(version: PolicyVersionId): PolicyLedger {
  return version === POLICY_VERSION ? LEGACY_POLICY : CURRENT_POLICY;
}
...
export function getTournamentPolicy(
  ...
  const tournament = CURRENT_POLICY.tournaments.find(
    ({ id }) => id === lineageId,
  );
